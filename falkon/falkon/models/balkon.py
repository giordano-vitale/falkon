...
from falkon.preconditioner import BalkonPreconditioner
...
class Balkon(FalkonBase):
    ...
    def init_pc(
        ...
        with TicToc(f"Calcuating Preconditioner of size {num_centers}", debug=self.options.debug):
            ...
            pc = BalkonPreconditioner(self.penalty, self.kernel, pc_opt)
            ny_weight_vec = None
    ...
    def run_solver(
        ...
        with TicToc("Computing Falkon iterations", debug=self.options.debug):
            ...
            beta, model_evolution = optim.solve(
                X, ny_pts, Y, self.penalty, initial_solution=warm_start, max_iter=self.maxiter, callback=cb
            )
            alpha = self.precond.apply(beta)

            if self.options.test_partial_iterations:
                pullback_start_time = time.time()
                for j in range(model_evolution.shape[1]):
                    model_evolution[:, j:j+1] = self.precond.apply(model_evolution[:, j:j+1])
                pullback_end_time = time.time()
                print(f"Time to apply B to the intermediate solutions (to remove from the total time of Balkon): {pullback_end_time - pullback_start_time} seconds")
            
        return alpha, beta, model_evolution
    ...
    def fit(
        ...
        with torch.autograd.inference_mode():
            ...
            alpha, beta, model_evolution = self.run_solver(_use_cuda_mmv, calc_kernel, X, Y, ny_points, warm_start, validation_cback)
            self.alpha_, self.beta_, self.model_evolution_, self.ny_points_ = alpha, beta, model_evolution, ny_points
        return self
