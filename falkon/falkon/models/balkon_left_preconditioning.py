...
from falkon.preconditioner import BalkonLeftPreconditioner
from falkon.sparse import SparseTensor
from falkon.utils import TicToc
from falkon.utils.devices import get_device_info

__all__ = ("BalkonLeftPreconditioning",)
...
class BalkonLeftPreconditioning(FalkonBase):
    ...
    def init_pc(
        ...
        with TicToc(f"Calcuating Preconditioner of size {num_centers}", debug=self.options.debug):
            ...
            pc = BalkonLeftPreconditioner(self.penalty, self.n, self.kernel, pc_opt)
            ny_weight_vec = None
    ...
    def run_solver(
        ...
        with TicToc("Computing Falkon iterations", debug=self.options.debug):
            ...
            optim = falkon.optim.BalkonLeftPreconditionedConjugateGradient(kernel, self.precond, o_opt, weight_fn=self.weight_fn)
            alpha, model_evolution = optim.solve(
    ...
    def fit(
        ...
        with torch.autograd.inference_mode():
            ...
            alpha, model_evolution = self.run_solver(_use_cuda_mmv, calc_kernel, X, Y, ny_points, warm_start, validation_cback)
            self.alpha_, self.model_evolution_, self.ny_points_ = alpha, model_evolution, ny_points
        return self
