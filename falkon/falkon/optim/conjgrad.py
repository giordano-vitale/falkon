...
from falkon.preconditioner.pc_utils import inplace_set_diag_th
...
class ConjugateGradient(Optimizer):
    ...
    def solve(
        ...
        X_evolution = torch.zeros((B.shape[0], max_iter+1), device="cpu", dtype=B.dtype)
        X_evolution[:, :1]  = X
        ...
        for self.num_iter in range(max_iter):
            with TicToc("Chol Iter", debug=False):
                ...
                X_evolution[:, self.num_iter+1:self.num_iter+2]  = X
                ...
                print(f"Iteration {self.num_iter+1} completed")
...
class PreconditionedConjugateGradient(Optimizer):
    ...
    def solve(
        ...
        B: torch.Tensor,
        mmv: Callable[[torch.Tensor], torch.Tensor],
        prec_action: Callable[[torch.Tensor], torch.Tensor],
        max_iter: int,
        callback: Optional[Callable[[int, torch.Tensor, float], None]] = None,
    ) -> tuple[torch.Tensor, torch.tensor]:
        """Implementation of PCG with left preconditioning
        """
        t_start = time.time()
        ...
        for self.num_iter in range(max_iter):
            with TicToc("Chol Iter", debug=False):
                ...
                MRnew = R.clone()
                MRnew = prec_action(MRnew)
                Rsnew = (R.T @ MRnew).squeeze()
                converged = torch.less(Rsnew, tol)
                if torch.all(converged):
                    break
                if diff_conv and torch.any(converged):
                ...
                # P = R + P @ diag(mul)
                multiplier = (Rsnew / Rsold.add_(m_eps)).reshape(1, -1)
                P = P.mul_(multiplier).add_(MRnew)
                Rsold = Rsnew
...
class FalkonConjugateGradient(Optimizer):
    ...
    def solve(self, X, M, Y, _lambda, initial_solution, max_iter, callback=None):
        ...
        with ExitStack() as stack, TicToc("ConjGrad preparation", False):
            ...
            beta, _ = self.optimizer.solve(initial_solution, B, mmv, max_iter, callback)
...
class BalkonConjugateGradient(Optimizer):
    ...
    def balkon_mmv(self, sol, penalty, X, M, n: int):
        ...
        with TicToc("MMV", self.params.debug):

            v = torch.empty_like(sol)
            v_t = torch.empty_like(sol)
            out = torch.empty_like(sol)

            num_blocks = M.shape[0] // prec.m0

            eps = prec.params.pc_epsilon(X.dtype)

            for i in range(num_blocks):
                i_start = i * prec.m0
                i_end = (i + 1) * prec.m0

                C, dA, dT = prec._build_block(prec.X_nys[i_start:i_end], eps)

                inplace_set_diag_th(C, dA)
                v[i_start:i_end] = trsm(sol[i_start:i_end], C, alpha=1.0, lower=1, transpose=1)

                inplace_set_diag_th(C, dT)
                v_t[i_start:i_end] = trsm(v[i_start:i_end], C, alpha=1.0, lower=0, transpose=0)
            ...
            for i in range(num_blocks):
                ...
                inplace_set_diag_th(C, dT)
                cc_[i_start:i_end] = trsm(cc_[i_start:i_end], C, alpha=1.0, lower=0, transpose=1)
                cc_[i_start:i_end].add_(v_[i_start:i_end])

                inplace_set_diag_th(C, dA)
                out[i_start:i_end] = trsm(cc_[i_start:i_end], C, alpha=1.0, lower=1, transpose=0)
    ...
    def solve(self, X, M, Y, _lambda, initial_solution, max_iter, callback=None):
        ...
        # Note that if we don't have CUDA this still works with stream=None.
        with ExitStack() as stack, TicToc("ConjGrad", self.params.debug):
            if cuda_inputs:
                stack.enter_context(torch.cuda.device(device))
                stack.enter_context(torch.cuda.stream(stream))
            y_over_n = Y / n  # Cannot be in-place since Y needs to be preserved
            ...
            # Run the conjugate gradient solver
            beta, model_evolution = self.optimizer.solve(initial_solution, B, mmv, max_iter, callback)

        return beta, model_evolution
...
class BalkonLeftPreconditionedConjugateGradient(Optimizer):
    ...
    def __init__(
        ...
        self.preconditioner = preconditioner
        self.params = opt
        self.optimizer = PreconditionedConjugateGradient(opt.get_conjgrad_options())

        self.weight_fn = weight_fn

    def mmv_H(self, sol, penalty, X, M, n: int):  # here mmv is only the action of H, independent of the preconditioner

        with TicToc("MMV", self.params.debug):

            # cc = self.kernel.dmmv(X, M, sol, None, opt=self.params)
            # cc_ = cc.div_(n)

            # v = self.kernel.mmv(M, M, sol)
            # v_ = v.mul_(penalty)
    ...
    def solve(self, X, M, Y, _lambda, initial_solution, max_iter, callback=None):
        ...
        with ExitStack() as stack, TicToc("ConjGrad", self.params.debug):
            ...
            if self.is_weighted:
                ...
                mmv = functools.partial(self.mmv_H, penalty=_lambda, X=X, M=M, n=n)

            # Preconditioner action of BalkonLeftPrecontitioner
            prec_action = prec.apply
            # Run PCG
            alpha, model_evolution = self.optimizer.solve(initial_solution, B, mmv, prec_action, max_iter, callback)

        return alpha, model_evolution
